...
@onready var objective_label = $HUD/ObjectiveLabel
@onready var sprite_2d = $HUD/Sprite2D
...
func _process(_delta):
	sprite_2d.texture = load(vecSprites[fruitIndex])
...
func removeFruitStack(index):
	typeStack.remove_at(index)
	positionStack.remove_at(index)
	#get_node("Farmer").refreshFruit()

func clearStacks():
	for _i in typeStack.size()-1:
		if (_i != 0):
			removeFruitStack(_i)
